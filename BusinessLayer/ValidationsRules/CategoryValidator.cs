...
namespace BusinessLayer.ValidationsRules
{
    public class CategoryValidator : AbstractValidator<Category>
    {
        ...
        {
            RuleFor(x => x.CategoryName).NotEmpty().WithMessage("Kategori adı boş geçemezsin");
            RuleFor(x => x.CategoryDescription).NotEmpty().WithMessage("Açıklamayı boş geçemezsin");
            RuleFor(x => x.CategoryName).MinimumLength(3).WithMessage("Lütfen en az 3 karater girin.");
